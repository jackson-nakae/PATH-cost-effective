...
def _load_df(value: Any, name: str) -> pd.DataFrame:
    if isinstance(value, pd.DataFrame):
        return value.copy()
    if isinstance(value, str):
        path = Path(value)
        if not path.exists():
            raise FileNotFoundError(f"{name} path not found: {value}")
        if path.suffix.lower() == ".parquet":
            return pd.read_parquet(path)
        if path.suffix.lower() in {".csv", ".txt"}:
            return pd.read_csv(path)
        raise ValueError(f"Unsupported file type for {name}: {path.suffix}")
    raise TypeError(f"{name} must be a DataFrame or file path string")


def _first_int_from_any(value: Any):
    ...
    if isinstance(value, str):
        ...
        try:
            ...
            try:
                ...
                return pd.NA
        if isinstance(parsed, (list, tuple, np.ndarray, pd.Series)) and len(parsed) > 0:
            try:
...
def _prepare_ce_and_plot_data_impl(
    ...
    if write_outputs:
        ...
        final_data.to_csv(output_dir / f"{output_prefix}_final_data.csv", index=False)
        hillslope_data_agg.to_csv(output_dir / f"{output_prefix}_hillslope_agg.csv", index=False)
        outlet_data_final.to_csv(output_dir / f"{output_prefix}_outlet_agg.csv", index=False)
        hillslope_char_data_agg.to_csv(output_dir / f"{output_prefix}_char_agg.csv", index=False)
...
def prepare_ce_and_plot_data(
    ...
    output_dir=".",
    output_prefix="catchment",
    legacy_gatecreek_format=False,
):
    """Public entry point for CE/plot preparation with optional Gatecreek legacy-format output."""
    hillslope_data_agg, outlet_data_final, hillslope_char_data_agg, final_data = _prepare_ce_and_plot_data_impl(
    ...
    if not legacy_gatecreek_format:
        return hillslope_data_agg, outlet_data_final, hillslope_char_data_agg, final_data

    hills_df = _load_df(hillslopes, "hillslopes")
    outlet_df = _load_df(contrasts, "contrasts")
    char_df = _load_df(hillslope_char, "hillslope_char")

    compat = final_data.copy()
    if "TopazID" in compat.columns:
        compat["TopazID"] = pd.to_numeric(compat["TopazID"], errors="coerce").astype("Int64")
    else:
        source_col = "topaz_ids_all" if "topaz_ids_all" in compat.columns else "topaz_ids"
        compat["TopazID"] = compat[source_col].apply(_first_int_from_any).astype("Int64")

    if "TopazID" not in hills_df.columns and "Topaz ID" in hills_df.columns:
        hills_df["TopazID"] = hills_df["Topaz ID"]
    if "TopazID" not in char_df.columns and "topaz_id" in char_df.columns:
        char_df["TopazID"] = char_df["topaz_id"]
    if "wepp_id" not in char_df.columns and "WeppID" in hills_df.columns:
        sbs_wepp_map = (
            hills_df[hills_df["scenario"] == "sbs_map"][["TopazID", "WeppID"]]
            .dropna()
            .drop_duplicates(subset=["TopazID"])
            .rename(columns={"WeppID": "wepp_id"})
        )
        char_df = char_df.merge(sbs_wepp_map, on="TopazID", how="left")
    ...
    char_keep = [
        ...
        if c in char_df.columns
    ]
    if char_keep:
        char_map = char_df[char_keep].copy()
        char_map["TopazID"] = pd.to_numeric(char_map["TopazID"], errors="coerce").astype("Int64")
        char_map = char_map.dropna(subset=["TopazID"]).drop_duplicates(subset=["TopazID"])
        compat = compat.merge(char_map, on="TopazID", how="left")

    if "contrast_scenario" not in outlet_df.columns and "contrast" in outlet_df.columns:
        outlet_df["contrast_scenario"] = outlet_df["contrast"].astype(str).str.split("to__").str[-1]

    if "contrast_topaz_id" in outlet_df.columns:
        outlet_df["contrast_topaz_id"] = pd.to_numeric(outlet_df["contrast_topaz_id"], errors="coerce").astype("Int64")
        out_key = "Avg. Ann. sediment discharge from outlet"

        sddc_post_cols = [c for c in final_data.columns if c.startswith("Sddc post-treat")]
        control_v_series = None
        for col in sddc_post_cols:
            label = col.replace("Sddc post-treat ", "")
            if "0.5 tons/acre" in col:
                scen = "mulch_15_sbs_map"
            elif "1 tons/acre" in col:
                scen = "mulch_30_sbs_map"
            elif "2 tons/acre" in col:
                scen = "mulch_60_sbs_map"
            else:
                continue

            out_scen = outlet_df[(outlet_df["key"] == out_key) & (outlet_df["contrast_scenario"] == scen)]
            if not out_scen.empty:
                out_map = out_scen.drop_duplicates(subset=["contrast_topaz_id"]).set_index("contrast_topaz_id")["v"]
                compat[f"Sddc post-treat {label}"] = compat["TopazID"].map(out_map)
                if control_v_series is None:
                    control_v_series = out_scen.drop_duplicates(subset=["contrast_topaz_id"]).set_index("contrast_topaz_id")["control_v"]
                    compat["Sddc post-fire"] = compat["TopazID"].map(control_v_series)

        sddc_red_cols = [c for c in final_data.columns if c.startswith("Sddc reduction")]
        for col in sddc_red_cols:
            label = col.replace("Sddc reduction ", "")
            compat[f"Sddc reduction {label}"] = compat["Sddc post-fire"] - compat.get(f"Sddc post-treat {label}", np.nan)

    if "TopazID" in hills_df.columns:
        hills_df["TopazID"] = pd.to_numeric(hills_df["TopazID"], errors="coerce").astype("Int64")
        hills_df["Landuse Area (ac)"] = pd.to_numeric(
            hills_df.get("Landuse Area (ac)", hills_df.get("Hillslope Area (ha)", np.nan)), errors="coerce"
        )
        if "Hillslope Area (ha)" in hills_df.columns and "Landuse Area (ac)" not in hills_df.columns:
            hills_df["Landuse Area (ac)"] = pd.to_numeric(hills_df["Hillslope Area (ha)"], errors="coerce") * 2.47105
        if "Sediment Yield (t/ac)" not in hills_df.columns and {"Sediment Yield (t)", "Landuse Area (ac)"}.issubset(hills_df.columns):
            hills_df["Sediment Yield (t/ac)"] = pd.to_numeric(hills_df["Sediment Yield (t)"], errors="coerce") / pd.to_numeric(
                hills_df["Landuse Area (ac)"], errors="coerce"
            )

        def _scenario_map(scenario_name, col_name):
            if col_name not in hills_df.columns:
                return pd.Series(dtype=float)
            subset = hills_df[hills_df["scenario"].eq(scenario_name)][["TopazID", col_name]].copy()
            subset["TopazID"] = pd.to_numeric(subset["TopazID"], errors="coerce").astype("Int64")
            return subset.dropna(subset=["TopazID"]).drop_duplicates(subset=["TopazID"]).set_index("TopazID")[col_name]

        runoff_col = "Runoff (mm)" if "Runoff (mm)" in hills_df.columns else "Runoff Depth (mm/yr)"
        lat_col = "Lateral Flow (mm)" if "Lateral Flow (mm)" in hills_df.columns else "Lateral Flow Depth (mm/yr)"
        base_col = "Baseflow (mm)" if "Baseflow (mm)" in hills_df.columns else "Baseflow Depth (mm/yr)"

        treat_scenarios = [("mulch_15_sbs_map", "0.5"), ("mulch_30_sbs_map", "1"), ("mulch_60_sbs_map", "2")]
        for scen, label in treat_scenarios:
            if not any(f"{label} tons/acre" in c for c in final_data.columns if "post-treat" in c or "reduction" in c):
                continue
            compat[f"Runoff {'0.5' if label == '0.5' else 'post-treat ' + label} tons/acre"] = compat["TopazID"].map(
                _scenario_map(scen, runoff_col)
            )
            compat[f"Lateralflow post-treat {label} tons/acre"] = compat["TopazID"].map(_scenario_map(scen, lat_col))
            compat[f"Baseflow post-treat {label} tons/acre"] = compat["TopazID"].map(_scenario_map(scen, base_col))

        compat["Runoff post-fire"] = compat["TopazID"].map(_scenario_map("sbs_map", runoff_col))
        compat["Runoff undisturbed"] = compat["TopazID"].map(_scenario_map("undisturbed", runoff_col))
        compat["Lateralflow post-fire"] = compat["TopazID"].map(_scenario_map("sbs_map", lat_col))
        compat["Lateralflow undisturbed"] = compat["TopazID"].map(_scenario_map("undisturbed", lat_col))
        compat["Baseflow post-fire"] = compat["TopazID"].map(_scenario_map("sbs_map", base_col))
        compat["Baseflow undisturbed"] = compat["TopazID"].map(_scenario_map("undisturbed", base_col))

        landuse_col = "Landuse" if "Landuse" in hills_df.columns else "Landuse Key"
        compat["Landuse"] = compat["TopazID"].map(_scenario_map("sbs_map", landuse_col))

    for label in ["0.5", "1", "2"]:
        for metric in ["Runoff", "Lateralflow", "Baseflow"]:
            treat_col = f"{metric} {'' if label == '0.5' else 'post-treat '}{label} tons/acre".strip()
            if treat_col in compat.columns:
                red_col = f"{metric} reduction {label} tons/acre"
                compat[red_col] = compat[f"{metric} post-fire"] - compat[treat_col]

    if "area" in compat.columns:
        compat["area"] = pd.to_numeric(compat["area"], errors="coerce") * 0.0001
    if "slope_scalar" in compat.columns:
        compat["slope_deg"] = np.degrees(np.arctan(pd.to_numeric(compat["slope_scalar"], errors="coerce")))

    compat["Landuse"] = pd.to_numeric(compat["Landuse"], errors="coerce").astype("Int64")
    high_severity = [105, 119, 129]
    mod_severity = [118, 120, 130]
    low_severity = [106, 121, 131]
    compat["Burn severity"] = "NaN"
    compat.loc[compat["Landuse"].isin(high_severity), "Burn severity"] = "High"
    compat.loc[compat["Landuse"].isin(mod_severity), "Burn severity"] = "Moderate"
    compat.loc[compat["Landuse"].isin(low_severity), "Burn severity"] = "Low"
    ...
    compat = compat.drop(columns=[c for c in drop_cols if c in compat.columns], errors="ignore")
    compat = compat.dropna().reset_index(drop=True)

    legacy_col_order = [
        "wepp_id",
        "TopazID",
        "Landuse",
        "Sdyd post-treat 0.5 tons/acre",
        "Sdyd post-treat 1 tons/acre",
        "Sdyd post-treat 2 tons/acre",
        "Sdyd post-fire",
        "Sdyd undisturbed",
        "Sddc post-treat 0.5 tons/acre",
        "Sddc post-treat 1 tons/acre",
        "Sddc post-treat 2 tons/acre",
        "Sddc post-treat 0.5 tons",
        "Sddc post-treat 1 tons",
        "Sddc post-treat 2 tons",
        "Sddc post-fire",
        "Runoff 0.5 tons/acre",
        "Runoff post-treat 1 tons/acre",
        "Runoff post-treat 2 tons/acre",
        "Runoff post-fire",
        "Runoff undisturbed",
        "Lateralflow post-treat 0.5 tons/acre",
        "Lateralflow post-treat 1 tons/acre",
        "Lateralflow post-treat 2 tons/acre",
        "Lateralflow post-fire",
        "Lateralflow undisturbed",
        "Baseflow post-treat 0.5 tons/acre",
        "Baseflow post-treat 1 tons/acre",
        "Baseflow post-treat 2 tons/acre",
        "Baseflow post-fire",
        "Baseflow undisturbed",
        "Sdyd reduction 0.5 tons/acre",
        "Sdyd reduction 1 tons/acre",
        "Sdyd reduction 2 tons/acre",
        "Sddc reduction 0.5 tons/acre",
        "Sddc reduction 1 tons/acre",
        "Sddc reduction 2 tons/acre",
        "Sddc reduction 0.5 tons",
        "Sddc reduction 1 tons",
        "Sddc reduction 2 tons",
        "Runoff reduction 0.5 tons/acre",
        "Runoff reduction 1 tons/acre",
        "Runoff reduction 2 tons/acre",
        "Lateralflow reduction 0.5 tons/acre",
        "Lateralflow reduction 1 tons/acre",
        "Lateralflow reduction 2 tons/acre",
        "Baseflow reduction 0.5 tons/acre",
        "Baseflow reduction 1 tons/acre",
        "Baseflow reduction 2 tons/acre",
        "centroid_lon",
        "centroid_lat",
        "area",
        "slope_deg",
    ...
    compat = compat[[c for c in legacy_col_order if c in compat.columns]]

    return hillslope_data_agg, outlet_data_final, hillslope_char_data_agg, compat
