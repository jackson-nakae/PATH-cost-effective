...
def ce_select_sites_2(data, treatments, treatment_cost, treatment_quantity, fixed_cost, sdyd_threshold, sddc_threshold, slope_range=(None), bs_threshold=(None)):
    import pandas as pd
    import numpy as np
    from pulp import LpProblem, lpSum, LpMaximize, LpMinimize, LpVariable, value, PulpSolverError, LpStatus, LpConstraint
    all_data = data.copy()
    ...
    sediment_yield_reduction_thresholds = sediment_yield_reduction_thresholds.clip(lower=0)
    ...
    for treatment, cost, quant in zip(treatments, treatment_cost, treatment_quantity)
}
    ...
    B = {
        t: LpVariable(f"B_{t}", 0, 1, cat='Binary')
        for t in treatments
    }
    ...
    model_primary += (
        sum(x[t][i] * water_quality.iloc[:, n].values[i]
    ...
    for i in range(num_sites):
        if max(soil_erosion.iloc[i,:]) > sediment_yield_reduction_thresholds[i]:
            ...
            model_primary += (
                sum(x[t][i] for t in treatments) == 0
            )
    ...
    try:
        ...
        if LpStatus[model_primary.status] != 'Optimal':
            ...
            x_2 = {
                t: [LpVariable(f"x_2_{t}_{i}", 0, 1, cat='Binary') for i in range(num_sites)]
                for t in treatments
            }

            B_2 = {
                t: LpVariable(f"B_2_{t}", 0, 1, cat='Binary')
                for t in treatments
            }

            #Objective function: Maximize the total sediment discharge reduction
            model_secondary += (
                        sum(x_2[t][i] * water_quality.iloc[:, n].values[i]
                            for n, t in enumerate(treatments)
                            for i in range(num_sites))
                    )
            # Constraint 1: One treatment per hillslope 
            ...
            for i in range(num_sites):
                if max(soil_erosion.iloc[i,:]) > sediment_yield_reduction_thresholds[i]:
                    ...
                    model_secondary += (
                        sum(x_2[t][i] for t in treatments) == 0
                    )
                # Ensure the most effective treatment is selected if no treatment can reduce Sdyd below threshold
                else:
                    model_secondary += (
            ...
            total_Sddc_reduction = sum(x_2[t][i].varValue * water_quality.iloc[:, n].values[i]
                        for n, t in enumerate(treatments)
                        for i in range(num_sites))
            ...
            if not missing_hillslopes.empty:
                missing_sdyd = missing_hillslopes[['wepp_id', 'Sdyd post-fire']].rename(columns={'Sdyd post-fire': 'final_Sdyd'})
            ...
            total_Sddc_reduction = sum(x[t][i].varValue * water_quality.iloc[:, n].values[i]
                        for n, t in enumerate(treatments)
                        for i in range(num_sites))
            final_Sddc = total_Sddc_postfire - total_Sddc_reduction
    ...
    return model_primary_status, treatment_cost_vectors, sediment_yield_reduction_thresholds, selected_hillslopes, treatment_hillslopes, total_Sddc_reduction, final_Sddc, hillslopes_sdyd, sdyd_df, untreatable_sdyd, total_cost, total_fixed_cost
...
def ce_select_sites_3(data, treatments, treatment_cost, treatment_quantity, fixed_cost, sdyd_threshold, sddc_threshold, slope_range=(None), bs_threshold=(None)):
    import pandas as pd
    import numpy as np
    from pulp import LpProblem, lpSum, LpMaximize, LpMinimize, LpVariable, value, PulpSolverError, LpStatus, LpConstraint
    all_data = data.copy()
    ...
    # slope = data['slope'].values
    # burn_severity = data['Burn severity'].values
    hillslope = data['contrast_id'].values
    #print(water_quality)
    #print(soil_erosion)
    # Calculate the sediment yield reduction needed for each hillslope to meet the threshold
    # If the sediment yield post-fire is below the threshold, then no reduction is needed (
...
# }
    treatment_cost_vectors = {
    treatment: data['area_sum'] * cost * quant
    for treatment, cost, quant in zip(treatments, treatment_cost, treatment_quantity)
}
    ...
    try:
        ...
        if LpStatus[model_primary.status] != 'Optimal':
            ...
            # create a list that contains the contrast_idt_id and the sediment yield of every hillslope after treatment or post-fire if no treatment was applied
            hillslopes_sdyd = []
            for i in range(num_sites):
                for t in treatments:
                    if x_2[t][i].varValue == 1:
                        hillslope = data['contrast_id'][i]
                        sdyd_value = data[f'Sdyd post-treat {t}'][i]
                        hillslopes_sdyd.append([hillslope, sdyd_value])
            for i in range(num_sites):
                if all(x_2[t][i].varValue == 0 for t in treatments):
                    ...
                    sdyd_value = data['Sdyd post-fire'][i]
                    hillslopes_sdyd.append([hillslope, sdyd_value])
            sdyd_df = pd.DataFrame(hillslopes_sdyd, columns=['contrast_id', 'final_Sdyd'])
            untreatable_sdyd = sdyd_df[sdyd_df['final_Sdyd'] > sdyd_threshold]
            
            #add back in the hillslopes that were not included in the optimization because they were filtered out by slope or burn severity
            missing_hillslopes = all_data[~all_data['contrast_id'].isin(data['contrast_id'])]
            if not missing_hillslopes.empty:
                missing_sdyd = missing_hillslopes[['contrast_id', 'Sdyd post-fire']].rename(columns={'Sdyd post-fire': 'final_Sdyd'})
                sdyd_df = pd.concat([sdyd_df, missing_sdyd], ignore_index=True)

            sdyd_df = sdyd_df.sort_values(by='contrast_id').reset_index(drop=True)
            ...
            # create a list that contains the contrast_id and the sediment yield of every hillslope after treatment or post-fire if no treatment was applied
            hillslopes_sdyd = []
            for i in range(num_sites):
                for t in treatments:
                    if x[t][i].varValue == 1:
...
def nb_select_sites(data, budget, treatments, treatment_cost, treatment_quantity, bio_vals, slope_range=(None), bs_threshold=(None)):
    import pandas as pd
    import numpy as np
    from pulp import LpProblem, lpSum, LpMaximize, LpMinimize, LpVariable, value, PulpSolverError, LpStatus, LpConstraint
    all_data = data.copy()
    ...
    water_quality, soil_erosion = benefits
    print(water_quality)
    # convert water_quality and soil_erosion values to their dollar values based on the bio_vals dictionary
    water_quality_val = water_quality * bio_vals['water_quality']
    print(water_quality_val)
    soil_erosion_val = soil_erosion * bio_vals['soil_erosion']
    ...
    # x[t][i] is a binary variable that indicates whether treatment t is applied to site i
    # B[t] is a binary variable that indicates whether treatment t was used thus applying any fixed costs related to treatment t
    model_primary = LpProblem("Select_Sites", LpMaximize)
    x = {
        t: [LpVariable(f"x_{t}_{i}", 0, 1, cat='Binary') for i in range(num_sites)]
        for t in treatments
    }

    # Define Objective function: Maximize sediment discharge and sediment yield reduction
    model_primary += (
        sum(x[t][i] * water_quality_val.iloc[:, n].values[i] for n, t in enumerate(treatments) for i in range(num_sites)) +
        sum(x[t][i] * soil_erosion_val.iloc[:, n].values[i] for n, t in enumerate(treatments) for i in range(num_sites))
    )
    ...
    for i in range(num_sites):
        model_primary += sum(x[t][i] * treatment_cost_vectors[t][i] for t in treatments) <= budget
    ...
    try:
        ...
        if LpStatus[model_primary.status] != 'Optimal':
            ...
            selected = [i for t in treatments for i in range(num_sites) if x[t][i].varValue == 1]
            if selected==[]:
                print("No sites selected, budget too low")
            ...
            # calculate the individual benefit to soil erosion and water quality and net benefit of the selected hillslopes
            water_quality_benefit = sum(x[t][i].varValue * water_quality_val.iloc[:, n].values[i]
                        for n, t in enumerate(treatments)
                        for i in range(num_sites))

            soil_erosion_benefit = sum(x[t][i].varValue * soil_erosion_val.iloc[:, n].values[i]
                        for n, t in enumerate(treatments)
                        for i in range(num_sites))
            
            total_benefit = sum(x[t][i].varValue * (water_quality_val.iloc[:, n].values[i] + soil_erosion_val.iloc[:, n].values[i])
                        for n, t in enumerate(treatments)
                        for i in range(num_sites))
            ...
            for i in range(num_sites):
                for t in treatments:
                    if x[t][i].varValue == 1:
                        hillslope = data['wepp_id'][i]
                        sdyd_value = data[f'Sdyd post-treat {t}'][i]
                        treated_hillslopes_sdyd.append([hillslope, sdyd_value])
                        hillslopes_sdyd.append([hillslope, sdyd_value])
            for i in range(num_sites):
                if all(x[t][i].varValue == 0 for t in treatments):
                    hillslope = data['wepp_id'][i]
    ...
    return treatment_cost_vectors, selected_hillslopes, treatment_hillslopes, total_Sddc_reduction, final_Sddc, treated_hillslopes_sdyd, hillslopes_sdyd, sdyd_df, total_cost, water_quality_benefit, soil_erosion_benefit, net_benefit, cost_benefit_ratio
...
def path_data_ag(hillslope_data, outlet_data, hillslope_char_data):
    ...
    required_hillslope_columns = ['WeppID', 'TopazID', 'Landuse', 'Soil', 'Length (m)',
       'Hillslope Area (ha)', 'Runoff (mm)', 'Lateral Flow (mm)',
       'Baseflow (mm)', 'Soil Loss (kg/ha)', 'Sediment Deposition (kg/ha)',
       'Sediment Yield (kg/ha)', 'scenario', 'Runoff (m^3)',
       'Lateral Flow (m^3)', 'Baseflow (m^3)', 'Soil Loss (t)',
       'Sediment Deposition (t)', 'Sediment Yield (t)', 'NTU (g/L)']
    required_outlet_data_columns = ['key', 'v', 'units', 'control_scenario', 'contrast_topaz_id',
       'contrast', '_contrast_name', 'contrast_id', 'control_v','control_units', 'control-contrast_v']
    required_hillslope_char_columns = ['topaz_id', 'slope_scalar', 'length', 'width', 'direction', 'aspect',
       'area', 'elevation', 'centroid_px', 'centroid_py', 'centroid_lon',
       'centroid_lat', 'wepp_id', 'TopazID']
   
    if not all(col in hillslope_data.columns for col in required_hillslope_columns):
        missing_columns = [col for col in required_hillslope_columns if col not in hillslope_data.columns]
        print("missing columns in hillslope data:", missing_columns)
    ...
    if not all(col in outlet_data.columns for col in required_outlet_data_columns):
        missing_columns = [col for col in required_outlet_data_columns if col not in outlet_data.columns]
        print("missing columns in outlet data:", missing_columns)
        raise ValueError("Outlet data file is missing required columns.")
    if not all(col in hillslope_char_data.columns for col in required_hillslope_char_columns):
        missing_columns = [col for col in required_hillslope_char_columns if col not in hillslope_char_data.columns]
        print("missing columns in hillslope characteristics data:", missing_columns)
        raise ValueError("Hillslope characteristics data file is missing required columns.")
    ...
    # Convert sdyd(t) to sdud(t/acre) in hillslope data
    hillslope_data['Landuse Area (ac)']= hillslope_data['Hillslope Area (ha)'].astype(float)* 2.47105
    hillslope_data['Sediment Yield (t/ac)'] = hillslope_data['Sediment Yield (t)'].astype(float) / hillslope_data['Landuse Area (ac)']

    out_mulch_60 = outlet_data.loc[(outlet_data['key'] == 'Avg. Ann. sediment discharge from outlet') & (outlet_data['contrast'].str.endswith('mulch_60_sbs_map'))].reset_index(drop=True)
    out_mulch_30 = outlet_data.loc[(outlet_data['key'] == 'Avg. Ann. sediment discharge from outlet') & (outlet_data['contrast'].str.endswith('mulch_30_sbs_map'))].reset_index(drop=True)
    out_mulch_15 = outlet_data.loc[(outlet_data['key'] == 'Avg. Ann. sediment discharge from outlet') & (outlet_data['contrast'].str.endswith('mulch_15_sbs_map'))].reset_index(drop=True)

    mulch_15= hillslope_data[hillslope_data['scenario']=='mulch_15_sbs_map'].reset_index(drop=True)
    mulch_30 = hillslope_data[hillslope_data['scenario']=='mulch_30_sbs_map'].reset_index(drop=True)
    mulch_60 = hillslope_data[hillslope_data['scenario']=='mulch_60_sbs_map'].reset_index(drop=True)
    sbs_map = hillslope_data[hillslope_data['scenario']=='sbs_map'].reset_index(drop=True)
    undisturbed = hillslope_data[hillslope_data['scenario']=='undisturbed'].reset_index(drop=True)
    
    combined_sdyd_df = pd.DataFrame({
        'wepp_id': sbs_map['WeppID'],
        'TopazID': sbs_map['TopazID'],
        'Landuse': sbs_map['Landuse'],
        'Sdyd post-treat 0.5 tons/acre': mulch_15['Sediment Yield (t/ac)'],
        'Sdyd post-treat 1 tons/acre': mulch_30['Sediment Yield (t/ac)'],
        'Sdyd post-treat 2 tons/acre': mulch_60['Sediment Yield (t/ac)'],
        'Sdyd post-fire': sbs_map['Sediment Yield (t/ac)'],
        'Sdyd undisturbed': undisturbed['Sediment Yield (t/ac)']
    })
    ...
    combined_sddc_df = pd.DataFrame({
        ...
        'Sddc post-treat 0.5 tons': out_mulch_15['v'],
        'Sddc post-treat 1 tons': out_mulch_30['v'],
        'Sddc post-treat 2 tons': out_mulch_60['v'],
        'Sddc post-fire': out_mulch_15['control_v']
    })



    data_piv = pd.merge(combined_sdyd_df, combined_sddc_df, on=['TopazID'], how='outer')\
    ...
    Sddc_reduction_15 = data_piv['Sddc post-fire'] - data_piv['Sddc post-treat 0.5 tons']
    Sddc_reduction_30 = data_piv['Sddc post-fire'] - data_piv['Sddc post-treat 1 tons']
    Sddc_reduction_60 = data_piv['Sddc post-fire'] - data_piv['Sddc post-treat 2 tons']

    #Sdyd_reduction_none = data_piv['Sdyd post-fire'] - data_piv['Sdyd post-treat None']
    Sdyd_reduction_15 = data_piv['Sdyd post-fire'] - data_piv['Sdyd post-treat 0.5 tons/acre']
    Sdyd_reduction_30 = data_piv['Sdyd post-fire'] - data_piv['Sdyd post-treat 1 tons/acre']
    Sdyd_reduction_60 = data_piv['Sdyd post-fire'] - data_piv['Sdyd post-treat 2 tons/acre']

    ntu_reduction_15 = data_piv['NTU post-fire'] - data_piv['NTU post-treat 0.5 tons/acre']
    ntu_reduction_30 = data_piv['NTU post-fire'] - data_piv['NTU post-treat 1 tons/acre']
    ntu_reduction_60 = data_piv['NTU post-fire'] - data_piv['NTU post-treat 2 tons/acre']

    runoff_reduction_15 = data_piv['Runoff post-fire'] - data_piv['Runoff 0.5 tons/acre']
    runoff_reduction_30 = data_piv['Runoff post-fire'] - data_piv['Runoff post-treat 1 tons/acre']
    runoff_reduction_60 = data_piv['Runoff post-fire'] - data_piv['Runoff post-treat 2 tons/acre']

    lateralflow_reduction_15 = data_piv['Lateralflow post-fire'] - data_piv['Lateralflow post-treat 0.5 tons/acre']
    lateralflow_reduction_30 = data_piv['Lateralflow post-fire'] - data_piv['Lateralflow post-treat 1 tons/acre']
    lateralflow_reduction_60 = data_piv['Lateralflow post-fire'] -  data_piv['Lateralflow post-treat 2 tons/acre']

    baseflow_reduction_15 = data_piv['Baseflow post-fire'] - data_piv['Baseflow post-treat 0.5 tons/acre']
    baseflow_reduction_30 = data_piv['Baseflow post-fire'] - data_piv['Baseflow post-treat 1 tons/acre']
    baseflow_reduction_60 = data_piv['Baseflow post-fire'] - data_piv['Baseflow post-treat 2 tons/acre']



    #data_piv['NTU reduction none'] = NTU_reduction_none
    data_piv['Sddc reduction 0.5 tons'] = Sddc_reduction_15
    data_piv['Sddc reduction 1 tons'] = Sddc_reduction_30
    data_piv['Sddc reduction 2 tons'] = Sddc_reduction_60
    ...
    data_piv['Runoff reduction 0.5 tons/acre'] = runoff_reduction_15
    data_piv['Runoff reduction 1 tons/acre'] = runoff_reduction_30
    data_piv['Runoff reduction 2 tons/acre'] = runoff_reduction_60

    data_piv['Lateralflow reduction 0.5 tons/acre'] = lateralflow_reduction_15
    data_piv['Lateralflow reduction 1 tons/acre'] = lateralflow_reduction_30
    data_piv['Lateralflow reduction 2 tons/acre'] = lateralflow_reduction_60

    data_piv['Baseflow reduction 0.5 tons/acre'] = baseflow_reduction_15
    data_piv['Baseflow reduction 1 tons/acre'] = baseflow_reduction_30
    data_piv['Baseflow reduction 2 tons/acre'] = baseflow_reduction_60
...
def path_data_ag_2(hillslope_data, outlet_data, hillslope_char_data):
    """
    Function that does the following:
      Checks for missing values, empty files, correct column names, and correct data types.
      Calculates and adds the reduction in sediment yield and NTU for each hillslope.
    ...
    # Check for correct column names
    required_hillslope_columns = ['scenario', 'Landuse Area (ha)', 'Runoff Depth (mm/yr)',
       'Lateral Flow Depth (mm/yr)', 'Baseflow Depth (mm/yr)',
       'Soil Loss (kg/yr)', 'Soil Loss Density (kg/ha/yr)',
       'Sediment Deposition (kg/yr)', 'Sediment Deposition Density (kg/ha/yr)',
       'Sediment Yield (kg/yr)', 'Sediment Yield Density (kg/ha/yr)',
       'Runoff (m^3)', 'Lateral Flow (m^3)', 'Baseflow (m^3)', 'Soil Loss (t)',
       'Sediment Deposition (t)', 'Sediment Yield (t)', 'NTU (g/L)', 'Slope',
       'Landuse Key']
    required_outlet_data_columns = ['key', 'value', 'units', 'v', 'control_scenario', 'contrast',
       '_contrast_name', 'contrast_id', 'control_v', 'control_units',
       'control-contrast_v']
    required_hillslope_char_columns = ['topaz_id', 'slope_scalar', 'length', 'width', 'direction', 'aspect',
       'area', 'elevation', 'centroid_px', 'centroid_py', 'centroid_lon',
       'centroid_lat', 'wepp_id']
    ...
    # Convert sdyd(t) to sdud(t/acre) in hillslope data
    hillslope_data['Landuse Area (ac)']= hillslope_data['Landuse Area (ha)'].astype(float)* 2.47105
    hillslope_data['Sediment Yield (t/ac)'] = hillslope_data['Sediment Yield (t)'].astype(float) / hillslope_data['Landuse Area (ac)']
    ...
    combined_sdyd_df = pd.DataFrame({
        'wepp_id': sbs_map['Wepp ID'],
        'TopazID': sbs_map['Topaz ID'],
        'Landuse': sbs_map['Landuse Key'],
        'Sdyd post-treat 0.5 tons/acre': mulch_15['Sediment Yield (t/ac)'],
        'Sdyd post-treat 1 tons/acre': mulch_30['Sediment Yield (t/ac)'],
        'Sdyd post-treat 2 tons/acre': mulch_60['Sediment Yield (t/ac)'],
        'Sdyd post-fire': sbs_map['Sediment Yield (t/ac)'],
    ...
    #NTU_reduction_none = data_piv['NTU post-fire'] - data_piv['NTU post-treat None']
    Sddc_reduction_15 = data_piv['Sddc post-fire'] - data_piv['Sddc post-treat 0.5 tons/acre']
    Sddc_reduction_30 = data_piv['Sddc post-fire'] - data_piv['Sddc post-treat 1 tons/acre']
    Sddc_reduction_60 = data_piv['Sddc post-fire'] - data_piv['Sddc post-treat 2 tons/acre']
