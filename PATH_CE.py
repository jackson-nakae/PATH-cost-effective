def _build_pulp_solver():
    """Return a PuLP solver instance, preferring in-process HiGHS on Windows.

    HiGHS via highspy avoids repeated console windows that appear with external
    CBC processes during threshold sweeps. Fallback remains CBC for compatibility.
    """
    try:
...
def ce_select_sites_2(data, treatments, treatment_cost, treatment_quantity, fixed_cost, sdyd_threshold, sddc_threshold, slope_range=(None), bs_threshold=(None)):
    ...
    #Feasibility check
    try:
        status = model_primary.solve(solver) if solver is not None else model_primary.solve()
        if LpStatus[model_primary.status] != 'Optimal':
            ...
            status_secondary = model_secondary.solve(solver) if solver is not None else model_secondary.solve()
